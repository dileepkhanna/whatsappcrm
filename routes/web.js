...
router.get("/get_web_public", async (req, res) => {
  try {
    // Force no cache to ensure browser gets fresh exchange_rate
    ...
    // Force exchange_rate to 1.0 for INR currency to prevent frontend multiplication
    const webData = data[0];
    if (webData && webData.currency_code === 'INR') {
      webData.exchange_rate = '1.0000';
    }
    
    res.json({ data: webData, addon: finalAddon || [], success: true });
  } catch (err) {
    res.json({ success: false, error: err, msg: "Server error", err });
    console.log(err);
  }
...
// Test endpoint to verify what the API is returning
router.get("/test_currency", async (req, res) => {
  try {
    const data = await query(`SELECT currency_code, currency_symbol, exchange_rate FROM web_public`, []);
    const webData = data[0];
    if (webData && webData.currency_code === 'INR') {
      webData.exchange_rate = '1.0000';
    }
    res.json({ 
      ...
      api_returns: webData,
      test: "2 × " + webData.exchange_rate + " = " + (2 * parseFloat(webData.exchange_rate))
    });
  } catch (err) {
